...
class RecommendationPageDiet extends StatefulWidget {
  @override
  _RecommendationPageDietState createState() => _RecommendationPageDietState();
}

class _RecommendationPageDietState extends State<RecommendationPageDiet> {
          ...
          child: Column(
            ...
            children: [
              ...
              ElevatedButton(
                onPressed: () {
                  setState(() {
                    selectedFoods = generateRecommendation1(1, 20, 7);
                  });
                },
                child: Text('추천 식단 보기'),
              ),
  ...
  List<String> generateRecommendation1(int start, int end, int count) {
    final random = Random();
    final menuList = [
      "그릭요거트", "매쉬드포테이토샐러드", "치킨스테이크 후무스 샐러드", "쉬림프 갈릭버터구이 샐러드", "베트남식 월남쌈 샐러드", "감바스 알 아히요 샐러드", "두부 포크소보로 샐러드",
      "불고기 부리또", "스리라차치킨 샐러드", "분짜 샐러드& 분짜 드레싱", "바질페스토 치킨샐러드& 발사믹 비네거 드레싱", "우삼겹덮밥&팽이장국 세트", "커리&닭안심가라아게 세트",
      "낙지볶음& 콩나물 세트", "고기듬뿍렌틸콩비빔밥 & 황태국", "고기듬뿍 유산슬 덮밥 & 얼큰두부백짬뽕탕 세트", "제육 오므라이스 & 두부양배추 샐러드", "브로콜리 참깨 두부 무침", "전복 현미리조또", "닭가슴살 죽",
